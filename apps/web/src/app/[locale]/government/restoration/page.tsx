...
export const metadata: Metadata = { title: "Restoration pipeline", description: "Sites marked under restoration or proposed for restoration." };

export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const metrics = db.metrics() as any;
  const items = db.heritage.restoration();
  return (
    <PortalShell portal={"Government"} title={"Restoration pipeline"} subtitle={"Sites marked under restoration or proposed for restoration."} nav={GOVERNMENT_NAV} active={"/government/restoration"} accent={"emerald"} roleNote={""}>
