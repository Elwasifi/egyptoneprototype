...
/** Villages template. One route serves every village recorded against a governorate. */
export default async function VillageDetail({ params }: { params: Promise<{ locale: string; slug: string }> }) {
          ...
          <InfoCard title="Other places here">
            <div className="flex flex-wrap gap-1.5">
              {gov.cities.filter((c) => c !== name).map((c) => (
                <Link key={c} href={l(`/villages/${gov.slug}-${c.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`)} className="rounded-full border border-white/10 px-2.5 py-1 text-[11.5px] text-ink-mid hover:border-gold-600/40 hover:text-gold-300">{c}</Link>
              ))}
            </div>
          </InfoCard>
